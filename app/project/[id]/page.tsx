...
export default async function ProjectPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const reviews = project.feedback.filter(f => !f.isBugReport);
  const bugReports = project.feedback.filter(f => f.isBugReport);
            ...
            {project.updates.map((update, idx) => (
              <div key={idx} className="bg-gray-900 rounded-lg p-4 border border-gray-800">
                <div className="flex justify-between items-center mb-2">
                  <span className="font-bold text-white">Version {update.version}</span>
        ...
        <div className="md:col-span-1">
          ...
          <div className="space-y-4">
            {project.screenshots.map((ss, idx) => (
              <div key={idx} className="bg-gray-800 aspect-[9/16] rounded-xl flex items-center justify-center text-gray-600 border border-gray-700 overflow-hidden">
                <span className="text-sm">Image: {ss}</span>
              </div>
            ))}
          </div>
        </div>
                ...
                <div key={review.id} className="bg-gray-900 rounded-xl p-5 border border-gray-800">
                  ...
                  {review.developerReply && (
                    <div className="mt-3 bg-gray-950 p-3 rounded-lg border-l-2 border-indigo-500 ml-4">
                      ...
                      <p className="text-gray-400 text-sm">{review.developerReply}</p>
                    </div>
                  )}
                </div>
              ))}
            ...
            <div className="space-y-6">
              {bugReports.map(bug => (
                <div key={bug.id} className="bg-gray-900 rounded-xl p-5 border border-red-900/30">
                  <div className="flex justify-between items-start mb-3">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-full bg-red-900/50 text-red-200 flex items-center justify-center font-bold text-sm">
                        {bug.user.charAt(0)}
                      </div>
                      <span className="font-semibold block">{bug.user}</span>
                    </div>
                    <span className="text-xs text-gray-500">{bug.date}</span>
                  </div>
                  ...
                  {bug.developerReply && (
                    <div className="mt-3 bg-gray-950 p-3 rounded-lg border-l-2 border-red-500 ml-4">
                      ...
                      <p className="text-gray-400 text-sm">{bug.developerReply}</p>
                    </div>
                  )}
                </div>
              ))}
